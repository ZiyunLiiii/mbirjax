...
class ConeBeamModel(mbirjax.TomographyModel):
    ...
    def _apply_ramp_filter_1d(self, projection_row, ramp_filter):
        ...
        Args:
            projection_row (jax array): A 1D array representing a single row of the sinogram.
            ramp_filter (jax array): The ramp filter generated by create_ramp_filter.

        Returns:
            filtered_row (jax array): The filtered projection row.
        """
        # Convolve the projection row with the ramp filter using 'valid' boundary condition
    ...
    def _apply_fdk_filter(self, sinogram):
        ...
        sinogram_flattened = sinogram.reshape(-1, num_channels)  # Shape: (num_views * num_rows, num_channels)

        # Apply ramp filter across all rows using a single vmap
        filtered_sinogram = jax.vmap(self._apply_ramp_filter_1d, in_axes=(0, None))(sinogram_flattened, ramp_filter)

        # Reshape the filtered sinogram back to (num_views, num_rows, num_channels)
